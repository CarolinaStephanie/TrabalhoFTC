...
bool isStringValid(string s, int i)
{
...
bool isLastStringValid(string s, int i)
{
...
/*
 Check if the symbol s belong to the string symbols
*/

bool checkIfIsInArray(char s)
{
    int i = 0;
    bool answ = false;
    while (i < qtdSymbols)
...
int reg_nfa(string s, int nfa_table[][col])
{
    ...
    for (int i = 0; i < l; i++)
    {
        ...
        if (isSymbolValid(x))
        {
            position = getPositionInArray(x);
            nfa_table[states][0] = states;
            init[a] = states;
            a += 1;
            states += 1;
            nfa_table[states - 1][position] = states;
            fin[b] = states;
            b += 1;
            nfa_table[states][0] = states;
            states += 1;
...
void eclosureAux(int qtd, int total, int nfa_table[][col], int y, stack<int> s, map<int, int> m)
{
    if (qtd <= total)
    {
        if (nfa_table[y][qtd] != -1)
        {
            s.push(nfa_table[y][qtd]);
            m[nfa_table[y][qtd]] = 1;
            eclosureAux(qtd + 1, total, nfa_table, y, s, m);
        }
    }
}
...
vector<int> eclosure(int nfa_table[][col], int x)
{
    ...
    while (!s.empty())
    {
        y = s.top();
        s.pop();
        eclosureAux(aux, qtdSymbols * 2, nfa_table, y, s, m);
    }
